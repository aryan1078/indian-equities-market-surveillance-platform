...
\begin{tikzpicture}[
    ...
    box/.style={draw=accent, rounded corners=3pt, fill=softaccent, minimum height=0.9cm, align=center, inner sep=6pt},
    store/.style={draw=black!65, rounded corners=3pt, fill=softgray, minimum height=0.9cm, align=center, inner sep=6pt},
    arrow/.style={-{Latex[length=2.5mm]}, thick}
]
\node[box] (provider) {Real market provider \\ (\texttt{yfinance}, provider adapter layer)};
\node[box, below=of provider] (collector) {Collector \\ normalization, backfill, capture replay, live poll};
\node[box, below=of collector] (kafka) {Kafka \\ \texttt{market\_ticks}, \texttt{anomaly\_detections}, DLQ};

\node[box, below left=1.2cm and 2.5cm of kafka] (storage) {Storage consumer};
\node[box, below=1.2cm of kafka] (anomaly) {Anomaly engine};
\node[box, below right=1.2cm and 2.5cm of kafka] (contagion) {Contagion engine};
...
\node[box, below=1.4cm of redis] (etl) {ETL \\ staging, cleansing, surrogate key assignment, view refresh};
\node[store, below=of etl] (warehouse) {PostgreSQL warehouse \\ dimensions, facts, materialized views};
\node[box, right=3.2cm of warehouse] (api) {FastAPI \\ operational APIs + warehouse APIs};
\node[box, above=of api] (frontend) {Next.js console \\ operator UI + analyst workbench};
...
\begin{tikzpicture}[
    ...
    step/.style={draw=black!60, rounded corners=3pt, fill=softgray, minimum width=0.95\textwidth, align=left, inner sep=7pt},
