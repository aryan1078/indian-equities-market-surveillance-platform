...
\begin{tikzpicture}[
    ...
    font=\small,
    box/.style={draw=accent, rounded corners=3pt, fill=softaccent, text width=0.24\textwidth, minimum height=0.9cm, align=center, inner sep=5pt},
    widebox/.style={draw=accent, rounded corners=3pt, fill=softaccent, text width=0.44\textwidth, minimum height=0.9cm, align=center, inner sep=5pt},
    store/.style={draw=black!65, rounded corners=3pt, fill=softgray, text width=0.24\textwidth, minimum height=0.9cm, align=center, inner sep=5pt},
    widestore/.style={draw=black!65, rounded corners=3pt, fill=softgray, text width=0.44\textwidth, minimum height=0.9cm, align=center, inner sep=5pt},
    arrow/.style={-{Latex[length=2.5mm]}, thick}
]
\node[widebox] (provider) {Real market provider \\ \texttt{yfinance} and provider adapter layer};
\node[widebox, below=of provider] (collector) {Collector \\ normalization, backfill, captured replay, live polling};
\node[widebox, below=of collector] (kafka) {Kafka \\ \texttt{market\_ticks}, \texttt{anomaly\_detections}, dead-letter flow};

\node[box, below left=0.95cm and 3.0cm of kafka] (storage) {Storage consumer};
\node[box, below=0.95cm of kafka] (anomaly) {Anomaly engine};
\node[box, below right=0.95cm and 3.0cm of kafka] (contagion) {Contagion engine};

\node[store, below=of storage] (cass) {Cassandra \\ \texttt{market\_ticks}, \texttt{anomaly\_metrics}, \texttt{latest\_market\_state}};
...
\node[widebox, below=0.95cm of redis] (etl) {ETL \\ staging, cleansing, surrogate key assignment, fact loading, view refresh};
\node[widestore, below=of etl] (warehouse) {PostgreSQL warehouse \\ dimensions, fact tables, materialized analytical views};
\node[widebox, below=of warehouse] (api) {FastAPI \\ operational APIs plus warehouse query APIs};
...
\begin{tikzpicture}[
    ...
    font=\small,
    step/.style={draw=black!60, rounded corners=3pt, fill=softgray, text width=0.88\textwidth, align=left, inner sep=6pt},
...
\node[step, below=of s3] (s4) {\textbf{4. Streaming scoring.} After the warm-up period, the anomaly engine computes return percentage, EWMA mean and variance, price \(z\)-score, volume \(z\)-score, rolling volatility, and a composite score.};
\node[step, below=of s4] (s5) {\textbf{5. Hot-state persistence.} The anomaly engine persists the metric to Cassandra, refreshes the Redis state and latest-view cache, records surveillance coverage in PostgreSQL, and emits anomaly detections to Kafka.};
\node[step, below=of s5] (s6) {\textbf{6. Contagion evaluation.} The contagion engine opens a five-minute observation window for a trigger stock, tracks sector peers, and writes a single relational event to PostgreSQL when the window closes.};
\node[step, below=of s6] (s7) {\textbf{7. End-of-day ETL.} The ETL service extracts anomaly metrics for a trading date, loads a staging table, derives contagion flags, assigns surrogate keys, loads warehouse facts, and refreshes analytical views.};
\node[step, below=of s7] (s8) {\textbf{8. User-facing analytics.} The FastAPI service serves live operational views from Redis/PostgreSQL and historical analysis from the warehouse. The frontend exposes these through operational dashboards and an analyst workbench.};
\draw[arrow] (s1) -- (s2);
\draw[arrow] (s2) -- (s3);
\draw[arrow] (s3) -- (s4);
\draw[arrow] (s4) -- (s5);
