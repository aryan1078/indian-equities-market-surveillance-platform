...
\begin{tikzpicture}[
    ...
    font=\small,
    box/.style={draw=accent, rounded corners=3pt, fill=softaccent, text width=3.0cm, minimum height=0.9cm, align=center, inner sep=5pt},
    widebox/.style={draw=accent, rounded corners=3pt, fill=softaccent, text width=5.8cm, minimum height=0.9cm, align=center, inner sep=5pt},
    store/.style={draw=black!65, rounded corners=3pt, fill=softgray, text width=3.1cm, minimum height=0.9cm, align=center, inner sep=5pt},
    widestore/.style={draw=black!65, rounded corners=3pt, fill=softgray, text width=5.8cm, minimum height=0.9cm, align=center, inner sep=5pt},
...
\draw[arrow] (provider.south) -- (collector.north);
\draw[arrow] (collector.south) -- (kafka.north);
\draw[arrow] (kafka.south west) -- (storage.north);
\draw[arrow] (kafka.south) -- (anomaly.north);
\draw[arrow] (anomaly.east) -- (contagion.west);
\draw[arrow] (storage.south) -- (cass.north);
\draw[arrow] (anomaly.south west) -- (cass.north east);
\draw[arrow] (anomaly.south) -- (redis.north);
\draw[arrow] (anomaly.south east) -- (pgop.north west);
\draw[arrow] (contagion.south) -- (pgop.north);
\draw[arrow] (cass.south east) -- (etl.north west);
\draw[arrow] (pgop.south west) -- (etl.north east);
\draw[arrow] (etl.south) -- (warehouse.north);
\draw[arrow] (redis.south) |- ([xshift=-1.5cm]api.north);
\draw[arrow] (pgop.south) |- ([xshift=1.5cm]api.north);
\draw[arrow] (warehouse.south) -- (api.north);
\draw[arrow] (api.south) -- (frontend.north);
\end{tikzpicture}
\caption{High-level system architecture}
\label{fig:architecture}
\end{figure}

This separation also reflects the way the warehouse is meant to be used. It is subject-oriented, because it is organized around stocks, sectors, dates, times, and contagion events rather than around application screens. It is integrated, because source-specific forms are normalized before loading. It is time-variant, because every fact is keyed by date and, where necessary, time. It is non-volatile in the analytical sense, because warehouse facts are loaded through ETL and then queried for analysis rather than updated continuously by business transactions.
...
\begin{tikzpicture}[
    ...
    y=1cm,
    font=\normalsize,
    step/.style={draw=black!60, rounded corners=3pt, fill=softgray, text width=6.2cm, minimum height=1.55cm, align=left, inner sep=7pt},
    arrow/.style={-{Latex[length=2.4mm]}, thick}
]
\node[step] (s1) at (0,0) {\textbf{1. Collection and normalization.} The collector downloads bars, converts timestamps to UTC and IST, adds exchange and sector metadata, assigns a deterministic dedupe key, and emits a canonical \texttt{MarketTick}.};
\node[step] (s2) at (7.5,0) {\textbf{2. Kafka publication.} The normalized tick is published to Kafka with the symbol as the message key so that per-symbol ordering is preserved.};
\node[step] (s3) at (7.5,-2.35) {\textbf{3. Dual consumption.} One consumer persists the raw tick to Cassandra. In parallel, the anomaly engine pulls the same tick and updates streaming statistics.};
\node[step] (s4) at (0,-2.35) {\textbf{4. Streaming scoring.} After the warm-up period, the anomaly engine computes return percentage, EWMA mean and variance, price \(z\)-score, volume \(z\)-score, rolling volatility, and the composite score.};
\node[step] (s5) at (0,-4.7) {\textbf{5. Hot-state persistence.} The anomaly engine writes the metric to Cassandra, refreshes Redis live state, records surveillance coverage in PostgreSQL, and emits anomaly detections to Kafka.};
\node[step] (s6) at (7.5,-4.7) {\textbf{6. Contagion evaluation.} The contagion engine opens a five-minute observation window, tracks anomalous sector peers, and writes one finalized contagion event to PostgreSQL.};
\node[step] (s7) at (7.5,-7.05) {\textbf{7. End-of-day ETL.} The ETL service extracts anomaly metrics for a trading date, loads staging, derives contagion flags, assigns surrogate keys, loads warehouse facts, and refreshes analytical views.};
\node[step] (s8) at (0,-7.05) {\textbf{8. User-facing analytics.} FastAPI serves live operational views from Redis/PostgreSQL and historical analysis from the warehouse, while the frontend exposes both operator and analyst workflows.};
\draw[arrow] (s1.east) -- (s2.west);
\draw[arrow] (s2.south) -- (s3.north);
\draw[arrow] (s3.west) -- (s4.east);
\draw[arrow] (s4.south) -- (s5.north);
\draw[arrow] (s5.east) -- (s6.west);
\draw[arrow] (s6.south) -- (s7.north);
\draw[arrow] (s7.west) -- (s8.east);
\end{tikzpicture}
...
This separation keeps operational truth and analytical truth distinct. Not every relational table belongs in the warehouse; some belong to the live system itself.
...
\begin{tikzpicture}[
    font=\footnotesize,
    dim/.style={draw=accent, rounded corners=3pt, fill=softaccent, text width=2.2cm, minimum height=0.9cm, align=center, inner sep=4pt},
    fact/.style={draw=black!70, rounded corners=3pt, fill=softgray, text width=2.45cm, minimum height=1.0cm, align=center, inner sep=4pt},
    link/.style={draw=black!55, line width=0.5pt}
]
\matrix[matrix of nodes,
        nodes={anchor=center},
        column sep=0.26cm,
        row sep=0.34cm] (m) {
    |[dim]| \shortstack{\textbf{Dimension} \\ \textbf{/ fact}} &
    |[fact]| \shortstack{\textbf{fact\_anomaly\_minute} \\ minute-grain signal facts} &
    |[fact]| \shortstack{\textbf{fact\_market\_day} \\ stock-day summary facts} &
    |[fact]| \shortstack{\textbf{fact\_contagion\_event} \\ event-grain contagion facts} &
    |[fact]| \shortstack{\textbf{fact\_surveillance\_coverage} \\ factless monitoring coverage} \\
    |[dim]| \textbf{dim\_stock} & $\bullet$ & $\bullet$ & $\bullet$ & $\bullet$ \\
    |[dim]| \textbf{dim\_sector} & $\bullet$ & $\bullet$ & $\bullet$ & \\
    |[dim]| \textbf{dim\_date} & $\bullet$ & $\bullet$ & $\bullet$ & $\bullet$ \\
    |[dim]| \textbf{dim\_time} & $\bullet$ & & & $\bullet$ \\
    |[dim]| \textbf{dim\_exchange} & $\bullet$ & $\bullet$ & & \\
};
\foreach \col in {2,3,4,5} {
    \draw[link] (m-1-\col.south) -- (m-6-\col.north);
}
\foreach \row in {2,3,4,5,6} {
    \draw[link] (m-\row-1.east) -- (m-\row-5.west);
}
\end{tikzpicture}
\caption{Warehouse dimensional model and fact participation}
...
These views correspond to the kinds of questions an analyst is likely to ask: Which sectors were under the most pressure? Which stocks remained persistently abnormal? What part of the trading session concentrated the highest signal intensity? Did contagion cluster around a few names or spread widely?
...
The system is strong enough to support a serious end-to-end demonstration, but the honest version of the project still has clear limitations.
...
That is the main contribution of the project. The system is not interesting because it uses many technologies. It is interesting because each storage choice is tied to a workload, each data transformation has a purpose, and the final application shows how distributed ingestion and dimensional analytics can coexist in one carefully designed product.
