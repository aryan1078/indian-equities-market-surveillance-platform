...
\small
\begin{tabularx}{\textwidth}{>{\bfseries}L{0.24\textwidth}Y}
Real market provider & \texttt{yfinance} through the shared provider adapter. Produces real daily and intraday market bars. \\
\(\downarrow\) Collector & Normalizes timestamps, attaches metadata, creates canonical ticks, and publishes events. \\
\(\downarrow\) Kafka & Replayable transport for \texttt{market\_ticks}, \texttt{anomaly\_detections}, and dead-letter messages. \\
\midrule
Storage consumer & Consumes \texttt{market\_ticks} and writes raw intraday bars to Cassandra. \\
Anomaly engine & Consumes \texttt{market\_ticks}, maintains Redis state, writes anomaly metrics, and emits detections. \\
Contagion engine & Consumes anomaly detections and writes finalized sector-spread events to PostgreSQL. \\
\midrule
Cassandra & Operational time-series store for ticks, anomaly metrics, and latest market state. \\
Redis & Hot state for EWMA recovery, latest snapshots, and freshness markers. \\
PostgreSQL operational & Relational operational data: runs, alerts, profiles, coverage, and contagion events. \\
\midrule
ETL & Extracts operational anomaly data, loads staging, assigns warehouse keys, loads facts, and refreshes views. \\
PostgreSQL warehouse & Dimensional warehouse with facts, dimensions, and materialized analytical views. \\
FastAPI and Next.js & Serve operational APIs, warehouse query APIs, the operator console, and the analyst workbench. \\
\end{tabularx}
\end{minipage}}
...
\centering
\small
\begin{tabularx}{\textwidth}{L{0.08\textwidth}L{0.23\textwidth}Y}
\toprule
\textbf{Step} & \textbf{Stage} & \textbf{What happens} \\
\midrule
1 & Collection and normalization & The collector downloads real market bars, converts timestamps to UTC and IST, attaches exchange and sector metadata, assigns a deterministic dedupe key, and emits a canonical \texttt{MarketTick}. \\
2 & Kafka publication & The normalized tick is published to Kafka with the symbol as the key, preserving per-symbol ordering for downstream consumers. \\
3 & Dual consumption & The storage consumer persists the raw tick to Cassandra while the anomaly engine consumes the same event for streaming statistics. \\
4 & Streaming scoring & After warm-up, the anomaly engine computes return percentage, EWMA mean and variance, price \(z\)-score, volume \(z\)-score, rolling volatility, and composite score. \\
5 & Hot-state persistence & The metric is written to Cassandra, the Redis state is refreshed, monitoring coverage is recorded in PostgreSQL, and anomaly detections are emitted to Kafka. \\
6 & Contagion evaluation & The contagion engine opens a five-minute sector window, tracks anomalous peers, and writes one finalized contagion event to PostgreSQL. \\
7 & End-of-day ETL & The ETL service extracts anomaly metrics for a trading date, loads staging, derives contagion flags, assigns surrogate keys, loads warehouse facts, and refreshes analytical views. \\
8 & User-facing analytics & FastAPI serves live operational views from Redis/PostgreSQL and historical analysis from the warehouse; the frontend exposes operator and analyst workflows. \\
\bottomrule
\end{tabularx}
...
\centering
\small
\begin{tabularx}{\textwidth}{L{0.18\textwidth}>{\centering\arraybackslash}p{0.10\textwidth}>{\centering\arraybackslash}p{0.12\textwidth}>{\centering\arraybackslash}p{0.12\textwidth}>{\centering\arraybackslash}p{0.11\textwidth}Y}
\toprule
\textbf{Dimension} &
\textbf{Minute} &
\textbf{Market day} &
...
\textbf{Purpose} \\
\midrule
\texttt{dim\_stock} & Yes & Yes & Yes & Yes & Symbol and company identity \\
\texttt{dim\_sector} & Yes & Yes & Yes & No & Sector roll-ups and peer groups \\
\texttt{dim\_date} & Yes & Yes & Yes & Yes & Trading-day analysis \\
\texttt{dim\_time} & Yes & No & No & Yes & Intraday minute analysis \\
\texttt{dim\_exchange} & Yes & Yes & No & No & Exchange-level attribution \\
\bottomrule
\end{tabularx}

\vspace{0.45em}
\footnotesize
Fact columns shown above refer to \texttt{fact\_anomaly\_minute}, \texttt{fact\_market\_day}, \texttt{fact\_contagion\_event}, and \texttt{fact\_surveillance\_coverage}. The table avoids mixed grain by keeping minute, day, event, and coverage facts separate.
